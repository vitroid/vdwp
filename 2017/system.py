...
for guest,cage in guestandcage:
    ...
    # f_c ######################
    if mol.dimen == 3:
        f_c[(guest,cage)] = numpy.array([histo2f.fvalue(histogram,T)+chempot.Avalue(T,mol.mass)+chempot.Cvalue(T,mol.moi[0], mol.moi[1], mol.moi[2])+chempot.SymmetryFix(T,mol.symm)+chempot.IntegrationFix(T,mol.dimen) for T in temperatures])
    elif mol.dimen == 1: #??
        errors.warn("Rodlike molecule is not implemented for now.")
    elif mol.dimen == 0:
        f_c[(guest,cage)] = numpy.array([histo2f.fvalue(histogram,T)+chempot.Avalue(T,mol.mass)+chempot.SymmetryFix(T,mol.symm)+chempot.IntegrationFix(T,mol.dimen) for T in temperatures])
...
plt.text(temperatures[0],mu_w2[0], "water")
