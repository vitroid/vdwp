...
temperatures = numpy.array(numpy.arange(263,295,3))
mode = "Graphs"
...
mu_i = numpy.array([crystals.U_e["Ih"] + vdWP.FreeEnergyOfVibration("lattice/Ih-Tanaka1998retry/cat.0.renma.dist", T) for T in temperatures])
# h_i ###################### (50)
...
if water_mode == "ANTOINE":
    #Antoine式のCPを使う
    mu_w = numpy.array([chempot.chempot(T,antoine.VaporPressure(T, A,B,C)) + chempot.IntegrationFix(T, tip4p.dimen)
             + chempot.StericFix(T, tip4p.mass, 2.0, tip4p.moi[0], tip4p.moi[1], tip4p.moi[2] )
             for T in temperatures])
elif water_mode == "GROMACS":
    #GROMACSによる純TIP4PのCPを使う
    mu_w = numpy.array([chempot.chempot(T,101326*math.exp(ip.value(T,TT,logP))) 
...
# mu_g ######################(15)
mu_g = numpy.array([chempot.chempot(T,p) + chempot.IntegrationFix(T, mol.dimen) + chempot.StericFix(T, mol.mass, mol.symm, mol.moi[0], mol.moi[1], mol.moi[2],debug=debug)  for T,p in zip(temperatures, pressures)])
# h_g ###################### (28)
h_g = dict()
mol = moldict[guest]
...
mu_e = numpy.array([ crystals.U_e[structure] + vdWP.FreeEnergyOfVibration(crystals.nma_file[structure], T) for T in temperatures ])
...
h_e = mu_e - temperatures* numpy.array(derivative.derivatives(temperatures,mu_e))
...
for cage in cages:
    ...
    # f_c ######################
    if histogram != None:
        f_c[cage] = numpy.array([histo2f.fvalue(histogram,T) + chempot.StericFix(T, mol.mass, mol.symm, mol.moi[0], mol.moi[1], mol.moi[2]) for T in temperatures])
        # h_c ###################### (35)
        errors.warn("Calculating enthalpy of guest "+mol.name+" in a "+("%d" % cage)+" hedral cage...")
        h_c[cage] = f_c[cage] - temperatures* numpy.array(derivative.derivatives(temperatures,f_c[cage])) # +  (mol.dof/2.0)*pc.NkB*temperatures
...
# mu_f ######################
sum = numpy.zeros_like(temperatures)
beta = 1.0/(pc.NkB*temperatures)
for cage in f_c:
    coeffs = -pc.NkB*temperatures*cages[cage]/Nw
    body  = numpy.log(1+numpy.exp(beta*(mu_g-f_c[cage])))
    add = coeffs*body
    sum = sum + add
Deltamu = sum
mu_f = mu_e + sum
...
h_f = mu_f - temperatures* numpy.array(derivative.derivatives(temperatures,mu_f))
...
if debug:
    # h_2 ###################### (49)
    h_2 = numpy.zeros_like(temperatures)
    beta = 1.0/(pc.NkB*temperatures)
    Nw        = crystals.cage_components[structure][0]
    print("cages / Nw = {0s} / {1d}" % (cages,Nw))
    for cage in f_c:
        e = numpy.exp(beta*(mu_g - f_c[cage]))
        y = e/(1.0+e)
...
h_3 = numpy.zeros_like(temperatures)
for cage in f_c:
    e = numpy.exp(beta*(mu_g - f_c[cage]))
    y = e/(1.0+e)
...
beta = 1.0/(pc.NkB*temperatures)
num_guest = numpy.zeros_like(temperatures)
for cage in f_c:
    e = numpy.exp(beta*(mu_g - f_c[cage]))
    y = e/(1.0+e)
...
""".split("\n"):
    ...
    if 1 < len(cols):
        ...
        T_w.append(a)
        H_w.append(b)
h_wMD = numpy.array([ip.value(T,T_w,H_w) for T in temperatures])
plt.figure(1)
...
""".split("\n"):
    ...
    if 1 < len(cols):
        ...
        T_w.append(a)
        H_w.append(b)
h_gMD = numpy.array([ip.value(T,T_w,H_w) for T in temperatures])
plt.figure(2)
plt.plot(temperatures,h_g,     label="cPentane (from BAR)")
plt.plot(temperatures,h_gMD,label="Gromacs CPENTANE")
plt.xlabel("Temperature [K]")
...
def LatentHeatMode():
    ...
    for guest in guests:
        ...
        if mp_struc != "":
          ...
          latent = interpolate.value( mp, temperatures, dh_fwpm )
