...
mu_g = np.array([chempot.chempot(T,p)
                 + chempot.IntegrationFix(T, mol.dimen)
...
for cage in cages:
    ...
    if histogram != None:
        ...
        f_c[cage] = np.array([histo2f.fvalue(histogram,T)
                              + chempot.IntegrationFix(T, mol.dimen)  #Added it 2016-2-15 is it correct?
                              + chempot.StericFix(T, mol.mass, mol.symm, mol.moi[0], mol.moi[1], mol.moi[2])
                              for T in temperatures])
        print(cage,":",f_c[cage],[chempot.IntegrationFix(T, mol.dimen) for T in temperatures])
        # h_k ###################### (35)
