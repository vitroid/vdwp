...
# system8 is an integration of system7_*.py.
...
if guest in dirs:
    temp, FE = Raw2Table(
        open("MDdata/neat/1atm/" + guest + "/FE.txt").readlines(), 0, 6
    )
    temp, dens = Raw2Table(
        open("MDdata/neat/1atm/" + guest + "/dens.txt").readlines(), 0, 1
    )
...
hosttype = host.split()[0]  # remove trailing spaces for TIP4P
dirname = hosttype + "+" + guest
if dirname in dirs:
    temp, FE = Raw2Table(
        open("MDdata/solution/1atm/" + dirname + "/FE.txt").readlines(), 0, 6
    )
    temp, FEw = Raw2Table(
        open("MDdata/solution/1atm/" + dirname + "/FEw.txt").readlines(), 0, 6
    )
    temp, dens = Raw2Table(
        open("MDdata/solution/1atm/" + dirname + "/dens.txt").readlines(), 0, 1
    )
    temp, H = Raw2Table(
        open("MDdata/solution/1atm/" + dirname + "/H.txt").readlines(), 0, 1
    )
    H = np.array(H)  # kJ/mol
    ...
    mu_s = np.array(
        [
            chempot.chempot(T, 101326 * math.exp(vdwp.value(T, temp, logP)))
            + chempot.IntegrationFixMinus(T, tip4p.dimen)
            + chempot.StericFix(T, tip4p.mass, 2.0, tip4p.moi)
...
mu_i = np.array(
    [
        crystals.U_e["Ih"]
        + normalmode.FreeEnergyOfVibration(
            "lattice/Ih-Tanaka1998retry/cat.0.renma.dist", T
        )
        for T in temperatures
    ]
)
...
if water_mode == "ANTOINE":
    ...
    mu_w = np.array(
        [
            chempot.chempot(T, antoine.VaporPressure(T, A, B, C))
            + chempot.IntegrationFixMinus(T, tip4p.dimen)
            + chempot.StericFix(T, tip4p.mass, 2.0, tip4p.moi)
            for T in temperatures
        ]
    ...
    mu_w = np.array(
        [
            chempot.chempot(T, 101326 * math.exp(vdwp.value(T, TT, logP)))
            + chempot.IntegrationFixMinus(T, tip4p.dimen)
            + chempot.StericFix(T, tip4p.mass, 2.0, tip4p.moi)
            for T in temperatures
        ]
    )
print(vdwp.value(270, temperatures, mu_w))

if debug:
    plt.plot(
        temperatures, [math.exp(vdwp.value(T, TT, logP)) * 101326 for T in temperatures]
    )
...
# h_w ###################### (50)
h_w = mu_w - temperatures * vdwp.derivatives(temperatures, mu_w)


# 水と氷の273.15 KでのCPを求める．
mu_w_std = vdwp.value(273.15, temperatures, mu_w)
mu_i_std = vdwp.value(273.15, temperatures, mu_i)
...
# h_i ###################### (50)
# ここで計算する．
h_i = mu_i - temperatures * vdwp.derivatives(temperatures, mu_i)
...
if guest not in status:
    logger.warning(f"Regard the guest in gas state at {pressure} Pa.")
    pressures = [pressure for T in temperatures]
else:
    if status[guest][0] == "IDEALGAS":
        logger.warning(f"Regard the guest in gas state at {status[guest][1]} Pa.")
        pressures = [status[guest][1] for T in temperatures]
    elif status[guest][0] == "LIQUID-ANTOINE":
        logger.warning(
            "Regard the guest in ideal gas at its vapor pressure specified by Antoine's eq."
        )
        A = status[guest][1]
        B = status[guest][2]
        C = status[guest][3]
        ...
        logger.warning(
            "Regard the guest in ideal gas at its vapor pressures given explicitly."
        )
        T = status[guest][1]
        logP = status[guest][2]
        pressures = [101326 * math.exp(vdwp.value(t, T, logP)) for t in temperatures]
print(pressures)
# mu_g ######################(15)
mu_g = np.array(
    [
        ...
        + chempot.StericFix(T, mol.mass, mol.symm, mol.moi, debug=debug)
...
h_g = mu_g - temperatures * np.array(vdwp.derivatives(temperatures, mu_g))


####### Clathrate terms ################################################

# for hydrate structure types
logger.info("Calculating chemical potential of empty clathrate " + structure + "...")
mu_e = np.array(
    [
        crystals.U_e[structure]
        + normalmode.FreeEnergyOfVibration(crystals.nma_file[structure], T)
        for T in temperatures
    ]
)
...
# 空のケージの潜熱
# h_e ###################### (50)
logger.info("Calculating enthalpy of empty clathrate " + structure + "...")
h_e = mu_e - temperatures * np.array(vdwp.derivatives(temperatures, mu_e))
...
for cage in cages:
    ...
    if histogram != None:
        logger.info(
            "Calculating free energy of guest "
            + mol.name
            + " in a "
            + ("%d" % cage)
            + " hedral cage..."
        ...
        # h_k ###################### (35)
        logger.info(
            "Calculating enthalpy of guest "
            + mol.name
            + " in a "
            + ("%d" % cage)
            + " hedral cage..."
        )
        h_k[cage] = f_c[cage] - temperatures * np.array(
            vdwp.derivatives(temperatures, f_c[cage])
        )  # +  (mol.dof/2.0)*pc.NkB*temperatures
...
mol = moldict[guest]
logger.info(
    "Calculating chemical potential of guest " + mol.name + " in " + structure + "..."
)
...
h_f = mu_f - temperatures * np.array(vdwp.derivatives(temperatures, mu_f))
...
if debug:
    for cage in f_c:
        ...
        if histogram != None:
            ...
            x = dof / 2.0
            w_k = (
                np.array([histo2f.energy(histogram, T) for T in temperatures])
                + temperatures * x * pc.NkB
            )
...
if guest in Hneat:
    ...
    h_fMD = h_e - num_guest * (h_gMD - h_k[cage])  # (56),(57)
    h_sMD = h_d + num_guest * h_gMD
...
dh_fwpgMD = (h_sMD - h_cMD) / num_guest
dh_fwpmMD = (h_sMD - h_cMD) / mass * 1000.0  # per kG
...
def TestMode(guest):
    ...
    for cage in (12, 14, 16):
        ...
        if histogram:
            ...
            print(
                ...
                "StericFix for {0}hedral cage".format(cage),
            )
            print("---")
    for cage in (14, 12, 16):
        if f_c.has_key((guest, cage)):
            ...
            print(
                math.exp(beta * (mu_g[guest][0] - f_c[(guest, cage)][0])),
                "exp(beta(mu_g - f_%d))" % cage,
            )
...
def EmptyLattices(guest=""):
    ...
    for struc in ("Ih", "sI", "sII"):
        if struc == "Ih":
            fvib = vdWP.FreeEnergyOfVibration(
                "data/Ih-Tanaka1998retry/cat.0.renma.dist", T
            )
        else:
            fvib = vdWP.FreeEnergyOfVibration(crystals.nma_file[struc], T)
        print(
            "{0}\t{1}\t{2}\t{3}".format(
                struc, crystals.U_e[struc], fvib, crystals.U_e[struc] + fvib
            )
        )
...
def GraphMode():
    ...
    mol = moldict[guest]
    plt.plot(
        temperatures, h_d, label="h_d Sep. water+{0} ({1})".format(mol.name, structure)
    )
    plt.text(
        temperatures[0], h_d[0], "Sep. water+{0} ({1})".format(mol.name, structure)
    )
    ...
    mol = moldict[guest]
    plt.plot(
        temperatures, h_f, label="h_f {0} {1} (vp+vdWP)".format(structure, mol.name)
    )
